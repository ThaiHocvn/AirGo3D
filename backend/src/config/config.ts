...
if (!env) {
  console.log(
    "You have not provided the NODE_ENV environment variable (e.g., development, production)."
  );
...
console.log(
  `Detected NODE_ENV = ${env}, so the app will use the environment file ${envFilename}`
);
...
if (!fs.existsSync(path.resolve(envFilename))) {
  ...
  console.log(
    `Please create the file ${envFilename} and refer to the content in the .env.example file.`
  );
  throw new Error(`Could not find the environment file ${envFilename}`);
}
